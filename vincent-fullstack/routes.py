...
from . import review_queue_bp
from .models import db, Document, RiskFlag, AuditEntry, Template
...
def _reviewer_name(fallback: str = "reviewer") -> str:
    """Prefer a logged-in user if your app has one (e.g. Flask-Login's
    current_user); fall back to a posted name for now."""
    try:
...
def _create_document(*, title, content, doc_type, source, extracted_fields=None,
    ...
    db.session.add(doc)
    db.session.flush()  # get doc.id before adding related rows
...
def queue_approve(doc_id):
    ...
    # TODO: trigger downstream action here (send, file, invoice, etc.)
    return jsonify(doc.to_dict())
...
def template_generate(template_id):
    ...
    if missing:
        ...
        doc.risk_level = "high"
        db.session.commit()
        _log(doc, "system", "flagged", "Unfilled template variables")

    return jsonify(doc.to_dict()), 201


# ===========================================================================
# Public, read-only client status page
# ===========================================================================
@review_queue_bp.route("/status/<public_token>", methods=["GET"])
def public_status(public_token):
    doc = Document.query.filter_by(public_token=public_token).first() or abort(404)
    return jsonify({
        "title": doc.title,
